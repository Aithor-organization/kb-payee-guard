...
_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        "counterparty_name": {
            "type": ["string", "null"],
            "description": (
                "🔴 **대금을 받는 쪽**(매도인 / Seller / 공급자 / Supplier / Vendor / Contractor /\n"
                "Beneficiary / Exporter / Consignor)의 정식 법인명. 없으면 null.\n"
                "\n"
                "🔴 **매수인(Buyer / Purchaser / 발주자 / Importer / Consignee)을 고르면 틀린 답이다.**\n"
                "이 값은 은행이 '누구에게 돈이 나가는가' 를 판정하는 데 쓴다. 돈을 **보내는** 쪽이\n"
                "아니라 **받는** 쪽이다.\n"
                "\n"
                "판별 순서:\n"
                "  1. `Seller:` / `매도인:` / `Supplier:` 같은 **역할 라벨**이 있으면 그 옆 법인명.\n"
                "  2. 라벨이 없으면 **은행 계좌·수취 정보를 제공하는 쪽**(그 계좌로 돈이 들어간다).\n"
                "  3. 그것도 없으면 물품·용역을 **인도하는** 쪽.\n"
                "\n"
                "⚠️ 한국 기업이 등장한다고 해서 그쪽을 고르지 마라 — 한국 기업이 **매수인**이고\n"
                "해외 공급자가 수취인인 구성이 흔하다. 역할 라벨을 그대로 따르라."
            ),
        },
...
🔴 counterparty 는 **대금을 받는 쪽**(Seller/매도인/Supplier/Vendor/Beneficiary/Exporter)이다.
   Buyer/매수인/발주자/Importer 를 고르면 **틀린 답**이다. 이 값으로 은행이
   "돈이 누구에게 나가는가" 를 판정하기 때문이다.
   · `Seller:` `매도인:` 같은 **역할 라벨**이 있으면 그것을 그대로 따른다.
   · 라벨이 없으면 **은행 계좌를 제시하는 쪽**(그 계좌로 돈이 들어간다)을 고른다.
   · 한국 기업이 보인다고 그쪽을 고르지 마라 — **한국 기업이 매수인이고 해외 공급자가
     수취인인 구성이 이 도구의 주 대상**이다. 역할 라벨이 우선이다.
