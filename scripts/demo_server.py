...
<section id=perf class=alt><div class=in>
  ...
  <div class=mt>
    ...
    <div class=mc><div class=v>90.0<span class=u>%</span></div>
      <div class=k>차단 <b>등급</b>을 가르는 비트를 사람이 라벨링한 <b>개발셋 30건</b>과 대조 —
        비순환이지만 독립 test set 은 아닙니다(같은 30건으로 개선함). 정규식 30.0%</div></div>
...
#    범위는 좁게 잡는다. `☀-➿` 를 통째로 지우면 이모지가 아닌 ♭(U+266D)·☎(U+260E)·
#    ✓(U+2713) 까지 조용히 사라진다 — 계약서 기호나 표 마크가 걸릴 수 있다.
#    astral 이모지 평면 + 실제로 쓰는 BMP 이모지만 열거한다.
_EMOJI = re.compile("[\U0001F300-\U0001FAFF"          # 이모지 평면 (🔴 🟡 💡 …)
                    "✅❌❎⚠⛔"  # ✅ ❌ ❎ ⚠ ⛔
                    "❗❓⭐⭕"        # ❗ ❓ ⭐ ⭕
                    "️]")                        # 변이 선택자
